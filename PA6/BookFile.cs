...
namespace PA6
{
    public class BookFile
    {
        ...
        {
            var jsonBookObject = JsonConvert.SerializeObject(myBook, new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
            var content = new StringContent(jsonBookObject, Encoding.UTF8, "application/json");
            string url;
            if (mode == "edit")
            {
